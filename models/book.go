...
type Book struct {
	...
	Writer      string    `orm:"column(writer);null"`
	ReleaseDate time.Time `orm:"column(release_date);type(timestamp with time zone);null"`
	IsReleased  bool      `orm:"column(is_released);null"`
}
